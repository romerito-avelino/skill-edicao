...
const PADROES_TEXTO_PRINCIPAL: RegExp[] = [
  ...
  /^heading/,        // heading — variante usada em ex. vfx-magnetic
  /^titulo$/,
  /^frase$/,
...
// Aceita sufixo "Text"/"Texto" para templates como morph-text (word1Text…word5Text)
function ehWordSequence(id: string): boolean {
  return /^word\d+(text)?$/i.test(id) || /^palavra\d+(texto)?$/i.test(id);
}
...
function escolherTemplatePrimario(tipoMomento: string): string {
  ...
  return opcoes[0] ?? TEMPLATES_FALLBACK[0];
}
...
export function aplicarAcaoAprovacao(
  ...
  switch (acao) {
    ...
    case "trocar_template": {
      ...
      let novasVars: Record<string, unknown> = {};
      if (novaMeta) {
        if (paleta) {
          novasVars = mapearVariaveis(novaMeta.variaveis, proposta.fraseImpacto, proposta.dadosVisuais, paleta, novoTemplate);
        } else {
          // Fallback: reaproveita valores quando os IDs coincidem
