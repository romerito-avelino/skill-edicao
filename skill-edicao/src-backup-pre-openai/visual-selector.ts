...
function descricaoEstiloImagem(estilo: number): string {
  const estilos: Record<number, string> = {
    1: "ilustração editorial conceitual, formas planas, metafórica",
    2: "aquarela, pinceladas suaves, textura de papel",
    3: "line art, traço limpo desenhado à mão",
    4: "personagens palito, figuras simples, cena narrativa",
    5: "storyboard estilo quadrinhos, contornos marcados",
  };
...
function estiloImagemEN(estilo: number): string {
  const estilos: Record<number, string> = {
    1: "flat editorial illustration, conceptual, minimal bold shapes, limited palette, NON-photorealistic",
    2: "watercolor illustration, soft washes, paper texture, hand-painted, NON-photorealistic",
    3: "clean line art, hand-drawn ink, minimal linework, NON-photorealistic",
    4: "minimalist stick-figure illustration, simple line figures, storytelling scene, NON-photorealistic",
    5: "comic storyboard illustration, bold ink outlines, flat colors, NON-photorealistic",
  };
  return estilos[estilo] || estilos[1];
}
